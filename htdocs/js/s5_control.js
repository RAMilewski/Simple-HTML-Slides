/* S5 Client for Slides with Numeric Ids. - ver 0.1  */
...
var serverURL="http://netfools.com/s5/";	/* Where to find the SlideSync Server              */ 
...
(function ($) {
		...
		$.fn.vAlign = function() {
			return this.each(function(i){
				...
				var overlap = Math.min(0,parseInt($(this).parent().css('margin-bottom'))); 
...
$(document).ready(function() {
	...
	var sseServer  = serverURL+"s5sse.php";   
	var useSSE = false;
	var loopTime = 1500; // XHR Loop Time when SSE is disabled
	var selector;
	...
	$('.slide').fadeIn(2000, function() {
		$('.slideBody').vAlign();  /* Vertically center all the vCenter divs. */ 
		$("#0").zoomTo({targetsize:zoomsize, duration:3000});
	});
	...
	*/
	...
	var onMessageHandler = function (event) {  //User when following via SSE
				...
				if (json.result != prevstatus) {
					...
					switch(json.result) {
						...
						default: //everything else (slide ids for now)
							...
							var slideID=parseInt(json.result.toString().replace("#",""));
						    slideClick(slideID);
						break;
					}
				}
    ...
    var message = "<p>This slide presentation is currently being controlled by one or more presenters. "+ 
   				  "Do you want to follow along?</p><p>Answering 'No' will give you exclusive local control of the "+ 
   ...
   /* Take Master Control  */
   	...
   	function takeControl() {
			...
			if (!master) {
				var message  = "You may take control of this presentation <br>"+
				...
				var message  = "You currently have control of this presentation <br>"+
	 ...
	 function mastercontrol(v,m,f) {
	 	...
	 	$.getJSON(server, {url: urlhash, control: masterkey, action: 'zoom', status: tempSlide}, 
			function(json) { if (json.sync != "ok") { alert("Master Control update failed \n " + json.error);}}
		);
		return false;
	 }
